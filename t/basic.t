...
sub run {
...

    {
        ok my $xd = XML::Dumb->new(
            root_wrapper          => "preise",
            children_as_attr_when => [ sub { $_[0]->tag eq 'preise' or $_[0]->tag eq 'domains' } ],
            only_child_as_attr    => [ sub { return 'preis' if $_[0]->parent and $_[0]->parent->tag eq 'domains' } ],
        );
        ok $xd->parsefile( "corpus/preise.xml" );
    ...
    {
        ok my $xd = XML::Dumb->new( children_as_attr_when => [ sub { $_[0]->tag eq 'preise' } ] );
        ok $xd->parse( "<preise meep='1' />" );
        like( exception { $xd->to_perl }, qr/tag with children as atts cannot have additional atts/ );
    }
}
